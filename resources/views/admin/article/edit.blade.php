                ...
                <form action="/articles/{{$article->id}}" method="post" enctype="multipart/form-data">
                    ...
                    <ul class="forminfo">
                        @if(count($errors)> 0 )
                            ...
                            <li><label>文章标题<b>*</b></label><input name="title" type="text" class="dfinput"
                                                                  value="{{$article->title}}" placeholder="请输入文章标题"
                                                                  style="width:518px;"/></li>

                            <li><label>所属分类<b>*</b></label>
                                <div class="vocation">
                                    <select class="select1" name="cate_id" id="columns" onchange="show(this)">
                                        <option value="">请选择分类</option>
                                        @foreach($columns as $item)
                                            <option value="{{$item->id}}"
                                                    @if($article->column_id == $item->id) selected @endif>@if($item->pid > 0)
                                                    &nbsp;|-@endif{{$item->column_name}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </li>
                            <li><label>封面图片<b>*</b></label>
                                ...
                                <div class="input-append">
                                    <input id="photoCover" readonly class="input-large" type="text" name="face_path"
                                           style="height:30px;width:265px;border:solid 1px #ccd0d2">
                                    <a class="btn" onclick="$('input[id=lefile]').click();">选择图片</a>
                                </div>
                            </li>
                            <li id="skill" style="display: none"><label>擅长技术<b>*</b></label><input name="skills" type="text" class="dfinput"
                                                                  value="{{$article->skills}}" placeholder="请输入文章来源"
                                                                  style="width:518px;"/></li>
                            <li><label>文章来源<b>*</b></label><input name="from" type="text" class="dfinput"
                                                                  value="{{$article->from}}" placeholder="请输入文章来源"
                                                                  style="width:518px;"/></li>
                            <li><label>通知内容<b>*</b></label>
                                <div><textarea id="editor" name="article_content"
                                               style="width:1000px;margin-left: 85px;z-index:-1">{{$article->content}}</textarea>
                                </div>
                            </li>
