...
@section('content')
    ...
    <div class="formbody">
        ...
        <form id="from" action="/columns/{{$info->id}}" method="post" enctype="multipart/form-data">
            ...
            <div id="usual1" class="usual">
                ...
                <div id="tab1" class="tabson">
                    <ul class="forminfo">
                        ...
                        <li><label>上级栏目<b>*</b></label>
                            <div class="vocation">
                                <select class="select1" name="pid">
                                    ...
                                    <option value="{{$item['id']}}" @if($info->pid == $item['id']) selected @endif>{{$item['column_name']}}</option>
                                        @foreach($item['child'] as $value)
                                            <option value="{{$value->id}}" disabled>&nbsp;|--{{$value->column_name}}</option>
                                        @endforeach
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <li><label>栏目名称<b>*</b></label><input name="column_name" type="text" class="dfinput"
                                                              placeholder="请填写栏目名称" value="{{$info->column_name}}" style="width:345px;"/></li>
                        <li><label>栏目图标<b>*</b></label>
                            <input id="lefile" name="ico" type="file" style="display:none">
                            <div class="input-append">
                                <input id="photoCover" readonly value="{{old('ico_path')}}" class="input-large" type="text" name="ico_path" style="height:30px;width:265px;border:solid 1px #ccd0d2">
                                <a class="btn" onclick="$('input[id=lefile]').click();">选择图片</a>
                            </div>
                        </li>
                        <li><label>栏目描述<b>*</b></label>

                            <textarea name="desc" style="width:345px;height:150px;border: solid 1px blue">{{$info->desc}}</textarea>
